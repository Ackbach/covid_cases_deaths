...
df = df[['date', 'state', 'deathConfirmed', 'positive']]
...
def analyze_state(state: str) -> list:
    ...
    while i < len(state_df):
        ...
        if friday_i < len(state_df):
            ...
            state_df['deathConfirmed'].iloc[saturday_i] = (2 / 3.) * state_df['deathConfirmed'].iloc[friday_i] \
                + (1 / 3.) * state_df['deathConfirmed'].iloc[monday_i]
            state_df['deathConfirmed'].iloc[sunday_i] = (1 / 3.) * state_df['deathConfirmed'].iloc[friday_i] \
                + (2 / 3.) * state_df['deathConfirmed'].iloc[monday_i]
    ...
    for i in range(len(state_df) - 1):
        ...
        state_df['deaths dy'].iloc[i] = state_df['deathConfirmed'].iloc[i] - state_df['deathConfirmed'].iloc[i+1]
